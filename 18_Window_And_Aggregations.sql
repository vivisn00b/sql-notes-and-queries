...
SELECT
    ...
    Sales,
    SUM(Sales) OVER () AS TotalSales,
    ROUND(CAST(Sales AS FLOAT) / SUM(Sales) OVER () * 100, 2) AS PercentageOfTotal     -- SUM(Sales) OVER (): calculates the total but does not collapse rows, so you can still use Sales from each row in the same query
                                                                                       -- SUM(Sales):will throw error as it is an aggregate, but you’re mixing it with non-aggregated columns (Sales) without GROUP BY
FROM Sales.Orders
